...
class Balance(ActFactory):
    ...
    # 控制技能
    Cyclone = None  # 旋风, 控制技能, 使目标无法做任何动作, 也无法被攻击
    Entangling_Roots = None  # 纠缠根须, 控制技能, 使目标无法移动
    Hibernate = None  # 催眠, 控制技能, 只能对动物和龙类使用
    Nature_s_Grasp = None  # 自然之握, 使你被击中后自动召唤纠缠根须定身攻击者

    # 其他
    Barkskin = None  # 树皮术
    Faerie_Fire = None  # 精灵之火, 破隐形, 降低护甲
    Innervate = None  # 激活, 为目标回复大量法力
    Thorns = None  # 荆棘术
    Soothe_Animal = None  # 安抚动物, 使目标的仇恨范围降低
...
class Restoration(ActFactory):
    ...
    Revive = None  # 复活
    Rebirth = None  # 战斗复活
    Tranquility = None  # 宁静, 长 CD 小队治疗


class Feral(ActFactory):
    # 熊形态
    Bear_FormShapeshift = None  # 熊形态
    ...
    Flight_FormShapeshift = None  # 飞行形态
