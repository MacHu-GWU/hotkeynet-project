...
class CommandBuilder:
    def __init__(self):
        self.cmd_launch_and_rename_game_client = None  # type: Command
        self.cmd_launch_and_rename_all_game_client = None  # type: Command
        self.cmd_bring_window_to_foreground = None  # type: Command
        self.cmd_resize_and_relocate_window = None  # type: Command
        self.cmd_center_overlap_layout = None  # type: Command
        self.cmd_enter_username_and_password = None  # type: Command
        self.cmd_batch_login = None  # type: Command

    def build_cmd_launch_and_rename_game_client(self, script: Script, config: Config):
        """
        运行一个游戏客户端, 并重命名游戏窗口.
        ...
        """
        self.cmd_launch_and_rename_game_client = Command(
    ...
    def build_cmd_launch_and_rename_all_game_client(self, script: Script, config: Config):
        """
        批量启动多个游戏窗口并全部重命名.
        ...
        无
        """
        self.cmd_launch_and_rename_all_game_client = Command(
            name="LaunchAndRenameAllGameClient",
            actions=[
                self.cmd_launch_and_rename_game_client.call("Local", window.title)
                for window in window_list[:config.game_client_config.n_windows]
            ],
            script=script,
        )

    def build_cmd_bring_window_to_foreground(self, script: Script, config: Config):
        """
        将某个窗口作为当前焦点窗口, 带到最前端.
        ...
        """
        self.cmd_bring_window_to_foreground = Command(
    ...
    def build_cmd_resize_and_relocate_window(self, script: Script, config: Config):
        """
        将某个窗口尺寸调整, 给左边留出一竖条用于 Control Panel, 并移动到屏幕中心区域.
        Resize and Relocate the Specified Window to the center.
        """
        self.cmd_resize_and_relocate_window = Command(
    ...
    def build_cmd_center_overlap_layout(self, script: Script, config: Config):
        """
        让所有窗口都符合 ResizeAndRelocateWindow 中的尺寸和位置设置
        Apply ResizeAndRelocateWindow to all window
        """
        self.cmd_center_overlap_layout = Command(
            name="CenterOverlapLayout",
            actions=[
                self.cmd_resize_and_relocate_window.call(window.title)
                for window in window_list[:config.game_client_config.n_windows]
            ],
            script=script,
        )

    def build_cmd_enter_username_and_password(self, script: Script, config: Config):
        """
        在指定游戏客户端窗口内填入账号密码.
        ...
        - %3%: 密码
        """
        self.cmd_enter_username_and_password = Command(
    ...
    def build_cmd_batch_login(self, script: Script, config: Config):
        """
        根据 Active Character 批量登录游戏账号.
        """
        self.cmd_batch_login = Command(
            name="BatchLogin",
            actions=[
                render_template(
                    remove_indent("""
                    ...
                    cmd=self.cmd_enter_username_and_password
                ),
            ],
            script=script,
        )
...
def build_all(script: Script, config: Config):
    cb = CommandBuilder()
    for key in CommandBuilder.__dict__.keys():
        if key.startswith("build_cmd_"):
            getattr(cb, key)(script, config)
