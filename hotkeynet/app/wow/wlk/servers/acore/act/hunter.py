...
class BeastMastery(ActFactory):
    ...
    Aspect_of_Pact_or_dragon_hawk = SHIFT_(F)  # 豹群 和 龙鹰 守护相互切换
    """
    #showtooltip
    /castsequence !豹群守护, !龙鹰守护
    """
    Aspect_of_viper_or_dragon_hawk = SHIFT_(G)  # 蝮蛇 和 龙鹰 守护相互切换
    """
    #showtooltip
...
class Marksmanship(ActFactory):
    ...
    Hunter_s_Mark = CTRL_(G)  # 猎人印记
    Periodical_add_Hunter_s_Mark = KEY_6  # 猎人印记

    Viper_Sting = CTRL_(X)  # 蝮蛇钉刺, 吸蓝效果

    Arcane_Shot = None  # 奥术射击
    Concussive_Shot = Z  # 震荡射击
    Tranquilizing_Shot = SHIFT_(TAB)  # 宁神射击, 移除激怒效果

    Distracting_Shot = SHIFT_(OEM3_WAVE_OR_BACK_QUOTE)  # 扰乱, 使目标攻击你, 猎人的嘲讽
    Flare = None  # 照明弹, 侦测隐形
...
class Survival(ActFactory):
    ...
    Deterrence = SHIFT_(F1)  # 威慑, 100% 躲闪以及偏斜法术, 保命技能
    Disengage = SHIFT_(R)  # 逃脱, 向后跳, 位移技能
    Mongoose_Bite = None  # 近战攻击技能
    Raptor_Strike = None  # 猛禽以及, 近战攻击技能
    Feign_Death = ALT_(E)  # 假死
    ...
    Freezing_Arrow = ALT_(R)  # 冰冻箭, 远程版本的冰冻陷阱
    Freezing_Trap = CTRL_(E)  # 冰冻陷阱, 使目标无法做出任何动作, 控制技能
    Frost_Trap = CTRL_(E)  # 冰霜陷阱, 使进入范围内的敌人移动速度降低
    Wing_Clip = T  # 摔绊, 近战技能, 使目标减速
    ...
    Misdirection = ALT_(KEY_1)  # 误导, 使你下几次攻击造成的仇恨转移到指定目标身上

    Misdirection_Focus_Macro = ALT_(Z)  # 焦点误导宏
    """
...
class Healbot(ActFactory):
    HB_Misdirection = CN.right  # 误导, 使你下几次攻击造成的仇恨转移到指定目标身上
