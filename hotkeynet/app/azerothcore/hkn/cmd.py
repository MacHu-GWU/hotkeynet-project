...
"""
实现各种 HotkeyNet Command (命令). 其实就是 HotkeyNet 中的 "函数" 的概念
"""
...
class CommandMixin:
    def build_cmd_launch_and_rename_game_client(self: "HknScript"):
        """
        运行一个游戏客户端, 并重命名游戏窗口.
    ...
    def build_cmd_launch_and_rename_all_game_client(self: "HknScript"):
        """
        批量启动多个游戏窗口并全部重命名.
        ...
        with hk.Command(
            ...
            for window, _ in self.mode.login_window_and_account_pairs:
                self.cmd_launch_and_rename_game_client.call(
                    args=[
                        window.title,
                    ]
                )

    def build_cmd_bring_window_to_foreground(self: "HknScript"):
        """
        将某个窗口作为当前焦点窗口, 带到最前端.
    ...
    def build_cmd_center_overlap_layout(self: "HknScript"):
        """
        ...
        with hk.Command(
            ...
            for window, _ in self.mode.login_window_and_account_pairs:
                self.cmd_resize_and_relocate_window.call(
                    args=[
                        window.title,
                    ]
                )

    def build_cmd_enter_username_and_password(self: "HknScript"):
        """
        在指定游戏客户端窗口内填入账号密码.
    ...
    def build_cmd_batch_login(self: "HknScript"):
        """
    ...
    def build_cmd_batch_login_and_enter_game(self: "HknScript"):
        """
            ...
            for char in self.mode.active_chars:
                with hk.SendPC():
                    with hk.SendWin(window=char.window.title):
                        ...
                        hk.Wait.make(500)
                        # 选择角色
                        x, y = self.mode.game_client.get_choose_char_x_y(
                            nth=char.nth_char
                        )
                        (
                            hk.ClickMouse(button=hk.MouseButtonEnum.LButton.value)
    ...
    def build_command_mixin(self: "HknScript"):
